...
def test_node_creation_and_serialization() -> None:
    ...
    # while the node name remains the model's only public reference token.
    node = Node("N_1", 1, 2.5)
    ...
    assert repr(node) == "Node(name='N_1', x=1.0, y=0.0, z=2.5)"


def test_node_accepts_y_coordinate() -> None:
    node = Node("LeftBase", 1.0, 2.0, y=-3.5)
